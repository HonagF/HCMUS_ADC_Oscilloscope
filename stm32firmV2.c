...
/* USER CODE BEGIN PV */
#define SAMPLES_PER_CH    512   // 512 mẫu mỗi kênh
...
uint16_t Process_Signal(uint16_t* raw_data, float* vpp, float* freq, float* vavg, float* vrms, float* vamp) {
    ...
    *vrms = rms_raw * adc_to_volt;
    ...
    uint16_t mid = (uint16_t)avg_raw; //Ngưỡng cắt trung bình là trung bình ADC thô
    int first_cross = -1, second_cross = -1;

    for(int i=1; i<SAMPLES_PER_CH; i++) {
        // Tìm điểm cắt lên qua mức trung bình
        if(raw_data[i-1] < mid && raw_data[i] >= mid) {
